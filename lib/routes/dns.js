...
router.post('/', function(req, res) {
  ...
  route53.createRecord(req.body.HostedZone, req.body.rr, function(err, data) {
    if (err) {
...
router.put('/:rrName', function(req, res) {
  ...
  route53.alterRecord(req.body.HostedZone, req.body.rr, function(err, data) {
    if (err) {
      res.status(err.statusCode).send({status: 'FAIL', dns: err});
    } else {
...
router.delete('/:rrName', function(req, res) {
  ...
  route53.deleteRecord(req.body.HostedZone, req.body.rr, function(err, data) {
    if (err) {
      res.status(err.statusCode).send({status: 'FAIL', dns: err});
    } else {
      // TODO: Call getRecord() to ensure record has been deleted.  Should
...
router.get('/:rrName', function(req, res) {
  ...
  route53.getRecord(req.body.HostedZone, req.params.rrName, req.body.rr.Type, function(err, data) {
    if (err) {
      console.log(err)
      res.status(err.statusCode).send({status: 'FAIL', dns: err});
    } else {
